...
namespace Loot.Modifiers.EquipModifiers.Defensive
{
	public class ManaShieldEffect : ModifierEffect
	{
		...
		{
			bool dodged = ModifierPlayer.Player(player.player).GetEffect<DodgeEffect>().Dodged;
			if (!dodged && !player.player.shadowDodge)
			{
				...
				if (manaBlock > 0 && player.player.statMana > 0)
				{
					...
					player.player.manaRegenDelay = Math.Max(player.player.manaRegenDelay, 90);
				}
	...
	public class ManaShield : EquipModifier
	{
		public override ModifierTooltipLine[] TooltipLines => new[]
		{
			...
			{
				Text = $"+{Properties.RoundedPower}% of damage taken is redirected to mana",
