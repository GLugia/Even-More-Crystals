...
namespace Loot.Modifiers
{
	...
	public abstract class WeaponModifier : Modifier
	{
		...
		public void UpdateUseDamage(Item item, Player player, int damageBefore, ref float add, ref float multi, ref float flat)
		{
			float tempDamage = add;
			ModifyWeaponDamage(item, player, ref add, ref multi, ref flat);
			float diff = tempDamage - damageBefore;
			add += diff;
		}
	}
}
