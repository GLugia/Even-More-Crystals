...
{
	public class ArcticCrystal : CrystalItem
	{
		internal override string CrystalName => "Arctic Crystal";
		protected override Color? OverrideNameColor => Color.LightSkyBlue;
		protected override string Usage => "freeze a random modifier";

		public override void SafeDefaults()
		{
			item.createTile = ModContent.TileType<ArcticCrystalTile>();
		}
		...
		protected override bool ModifyItem(Player player, Item item)
		{
			...
			if (actives != null
				&& actives.Count() > 0
				&& actives.Where(x => x.locked).Count() < actives.Count())
			{
				int id;
				...
				actives.ElementAt(id).locked = true;
